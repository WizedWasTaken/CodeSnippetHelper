...
export default function HamburgerMenu() {
    ...
    useEffect(() => {
        ...
        if (hamburgerRef.current) {
            ...
            hamburgerRef.current.style.marginTop = `${headerHeight + 2 - scrolledHeight
                }px`;
        }
    });
    ...
    return (
        <div className={`md:hidden cursor-pointer`}>
            ...
            >
                <div
                    className={`border-2 border-black dark:border-white w-9 transition-transform duration-300 ${menuOpen ? 'transform -rotate-45 translate-y-4' : ''
                        }`}
                ></div>
                <div
                    className={`border-2 border-black dark:border-white w-9 transition-transform duration-300 ${menuOpen ? '-translate-x-20' : 'translate-x-0'
                        }`}
                ></div>
                <div
                    className={`border-2 border-black dark:border-white w-9 transition-transform duration-300 ${menuOpen ? 'transform rotate-45 -translate-y-2' : ''
                        }`}
                ></div>
            </div>
            {/* Hamburger Menu Content */}
            <div
                className={`fixed p-5 left-0 w-full flex top-0 flex-end justify-end transition-translate duration-300 z-50 overflow-hidden ${menuOpen
            ...
            >
                <ul
                    className={`gap-2 w-full md:hidden duration-150 ${menuOpen ? 'flex translate-x-0 flex-col' : 'translate-x-full'
                        }`}
                    onClick={toggleHamburgerMenu}
                >
                    {menuOpen &&
